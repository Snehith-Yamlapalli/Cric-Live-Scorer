...
export default function Scorecard() {
  ...
  useEffect(() => {
    firebaserealtimedb
      .ref(`${userId}/${matchid}/${'Innings1'}/Totalteamruns`)
      .once('value')
      .then(snap => {
    ...
    firebaserealtimedb.ref(`${userId}/${matchid}/${Key}/batsmen/${striker}`)
      .once('value')
      .then(snap => {
        const d = snap.val()
        if (d) {
    ...
    firebaserealtimedb.ref(`${userId}/${matchid}/${Key}/batsmen/${nonstriker}`)
      .once('value')
      .then(snap => {
        const d = snap.val()
        if (d) {
    ...
    firebaserealtimedb.ref(`${userId}/${matchid}/${Key}/bowlers/${bowler}`)
      .once('value')
      .then(snap => {
        const d = snap.val()
        if (d) {
    ...
    firebaserealtimedb.ref(`${userId}/${matchid}/${Key}`)
      .once('value')
      .then(snap => {
        const d = snap.val()
        if (d) {
  ...
  function updatescore(val) {
    if (selected.length > 0) {
      ...
      const updates = {
        [`${matchid}/${Key}/Totalteamruns`]: teamruns + (val !== 'W' ? val : 0),
        [`${matchid}/${Key}/Totalteamwickets`]: teamwickets + (val === 'W' ? 1 : 0),
        [`${matchid}/${Key}/Totalteamovers`]: Teamovers + (newBowlerBalls === 6 ? 1 : 0),
        ...
        [`${matchid}/${Key}/Over/${bowler}/${bowlerovers}`]: updatedOver,

        [`${matchid}/${Key}/batsmen/${striker}`]: {
        ...
        [`${matchid}/${Key}/batsmen/${nonstriker}`]: {
  ...
  function addextras(selected, runs) {
    ...
    const updates = {
      [`${matchid}/${Key}/Totalteamruns`]: teamruns + val,
      [`${matchid}/${Key}/Totalteamwickets`]: teamwickets + (selected.includes('W') ? 1 : 0),
      [`${matchid}/${Key}/Totalteamovers`]: Teamovers + (newBowlerBalls === 6 ? 1 : 0),
      [`${matchid}/${Key}/Over/${bowler}/${bowlerovers}`]: selected,
      [`${matchid}/${Key}/batsmen/${striker}`]: {
    ...
    if (selected.includes('W') && newBowlerBalls !== 6) {
      ...
      sethisover(newOver);
      const updates =
        { [`${matchid}/${Key}/Over/${bowler}/${bowlerovers}`]: newOver }
